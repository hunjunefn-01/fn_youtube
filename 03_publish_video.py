...
def get_videos_from_playlist(youtube, playlist_id):
    ...
    playlist_request = youtube.playlistItems().list(
        part='snippet', #snippet이란 메타데이터이다. 동영상의 제목(title), 설명(description), 게시 시간(publishedAt), 썸네일(thumbnails), 카테고리 ID(categoryId) 등 다양한 메타 정보가 포함
...
def extract_name_ticker(filename, is_json=True): #파일이름이나 영상 제목에서 티커 정보 추출
    if is_json:
        ...
        # if len(parts) >= 2: # ['포바이포', '123.json'] → 2개 요소
        #     name = parts[0]
        #     ticker = parts[1]
        #     #print(f"Name {name} and ticker {ticker} is extracted from JSON.")
        #     return name, ticker
        if 2<= len(parts)<=3:
            name = parts[0]
            ticker = parts[1]
        ...
        parts = filename.split(' ') # 비디오 리스트에 올라갈때는 모든 특수문자가 공백으로 처리됌
        parts = list(filter(None, parts))  # 빈 문자열 제거, jyp. ent의 경우 ['JYP', 'Ent', '', '035900', 'shorts', 'with', 'tp'] parts 2번째에 문제가 생긴다
        # if len(parts) >= 2:
        #     name = parts[0]
        #     ticker = parts[1]
        #     return name, ticker
...
if __name__ == '__main__':
    ...
    main(run_today, run_am_or_pm)  # 메인 함수 실행
